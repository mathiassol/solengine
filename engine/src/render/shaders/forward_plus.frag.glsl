...
// Combines material unpacking (from vertex inputs + material textures) with full PBR lighting.
// Set 0: FrameUBO + 3 shadow samplers
// Set 1: material textures (albedo, normal_map, mr_map, emissive_map)
// Set 2: ssao + ibl_irradiance + ibl_prefilter + ibl_brdf_lut + depth_pre
...
layout(set = 0, binding = 0) uniform FrameUBO {
    ...
    vec4 u_ibl_params;    // x=enabled, y=intensity, z=diffuse_scale, w=specular_scale
};

layout(set = 0, binding = 1) uniform sampler2DArrayShadow s_shadow_map;
layout(set = 0, binding = 2) uniform sampler2DArray        s_shadow_raw;
layout(set = 0, binding = 3) uniform sampler2DArray        s_vsm;
...
layout(set = 2, binding = 0) uniform sampler2D   s_ssao;
layout(set = 2, binding = 1) uniform samplerCube s_ibl_irradiance;
layout(set = 2, binding = 2) uniform samplerCube s_ibl_prefilter;
layout(set = 2, binding = 3) uniform sampler2D   s_ibl_brdf_lut;
layout(set = 2, binding = 4) uniform sampler2D   s_depth_pre;

layout(push_constant) uniform PbrPush {
    mat4 model;
    vec4 base_color;
    // pbr.x=metallic  pbr.y=roughness  pbr.z=alpha_cutoff  pbr.w=alpha_mode(0=opaque,1=mask,2=blend)
    vec4 pbr;
...
float pcf_shadow(vec3 shadow_uvz, int cascade, float bias, float radius, mat2 rot) {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        sum += texture(s_shadow_map, vec4(shadow_uvz.xy + off, float(cascade), shadow_uvz.z - bias));
    }
...
float pcss_shadow(vec3 shadow_uvz, int cascade, float bias) {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        if (depth < receiver_z) { blocker_sum += depth; blocker_cnt++; }
    }
...
float vsm_shadow(vec3 shadow_uvz, int cascade) {
    ...
    if (t >= m.x) return 1.0;
    float var = max(m.y - m.x * m.x, u_shadow_extra.y);
    float d   = m.x - t;
    float p   = var / (var + d * d);
    float lo  = u_shadow_extra.x;
...
float sample_shadow(vec3 world_pos, vec3 normal, vec3 light_dir, int shadow_mode) {
    ...
    vec3 shadow_uvz; int cascade;
    if (!get_shadow_coord(world_pos, shadow_uvz, cascade)) return 1.0;
    float bias = compute_bias(normal, light_dir);
    if (shadow_mode == 2) return pcss_shadow(shadow_uvz, cascade, bias);
    else if (shadow_mode == 3) return vsm_shadow(shadow_uvz, cascade);
    else {
...
float contact_shadow(vec3 world_pos, vec3 normal, vec3 light_dir, float n_dot_l) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        float scene_d = texture(s_depth_pre, uv).r;
...
void main() {
    ...
    if (pc.flags.z > 0.5) {
        ...
        mat3 tbn = mat3(t, b, n);
        n = normalize(tbn * (texture(s_normal_map, v_uv).xyz * 2.0 - 1.0));
    }
    ...
    // SSAO: sample from screen-space position
    vec2 screen_uv = gl_FragCoord.xy / vec2(textureSize(s_ssao, 0));
    float ao = texture(s_ssao, screen_uv).r;
    ...
    if (dbg > 0) {
        ...
        else if (dbg == 6) out_color = vec4(emissive_color,        1.0);
        else if (dbg == 7) {
            // Cascade debug using depth_pre buffer
            float dep = texture(s_depth_pre, screen_uv).r;
            float view_z = -u_proj[3][2] / (dep + u_proj[2][2]);
            int cc = 3;
            if      (view_z > -u_cascade_splits.x) cc = 0;
            else if (view_z > -u_cascade_splits.y) cc = 1;
            else if (view_z > -u_cascade_splits.z) cc = 2;
            vec3 col;
    ...
    // ---- PBR lighting (unlit pass: output base + emissive directly) ----
    if (pc.flags.x < 0.5) {
        out_color = vec4(base + emissive_color, alpha);
        return;
    }
    ...
    for (int i = 0; i < light_count; ++i) {
        ...
        if (!shadow_used && type < 0.5 && u_shadow_config.x > 0.5) {
            ...
            shadow = sample_shadow(v_world_pos, n, l, smode);
            shadow_used = true;
            shadow *= contact_shadow(v_world_pos, n, l, n_dot_l);
    ...
    out_color = vec4(ambient + radiance + emissive_color, alpha);
}
